...
async function loadClockStats() {
    try {
        const res = await fetch(`${API}/api/clock/stats`);
        const data = await res.json();
        document.getElementById('total-clock').textContent = data.total.toLocaleString();
        ...
        document.getElementById('major-clock').textContent = major.toLocaleString();
        document.getElementById('minor-clock').textContent = minor.toLocaleString();
...
async function loadClockData() {
    try {
        const res = await fetch(`${API}/api/clock?limit=200`);
        allData = await res.json();

        let cleared = allData.filter(r => r.clearance_status === 'Cleared').length;
        document.getElementById('cleared-clock').textContent = cleared.toLocaleString();

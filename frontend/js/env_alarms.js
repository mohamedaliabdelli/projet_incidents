...
async function loadEnvStats() {
    try {
        const res = await fetch(`${API}/api/alarmes_environnement/stats`);
        const data = await res.json();
        document.getElementById('total-env').textContent = data.total.toLocaleString();
        ...
        document.getElementById('major-env').textContent = major.toLocaleString();
        document.getElementById('minor-env').textContent = minor.toLocaleString();
...
async function loadEnvData() {
    try {
        const res = await fetch(`${API}/api/alarmes_environnement?limit=200`);
        allData = await res.json();

        let cleared = allData.filter(r => r.clearance_status === 'Cleared').length;
        document.getElementById('cleared-env').textContent = cleared.toLocaleString();
...
function applyFilters() {
    ...
    if (site) filtered = filtered.filter(r => (r.alarm_source || '').toLowerCase().includes(site));

...
async function loadVswrStats() {
    try {
        const res = await fetch(`${API}/api/vswr/stats`);
        const data = await res.json();
        document.getElementById('total-vswr').textContent = data.total.toLocaleString();
        ...
        document.getElementById('major-vswr').textContent = major.toLocaleString();
        document.getElementById('minor-vswr').textContent = minor.toLocaleString();
...
async function loadVswrData(filters = {}) {
    try {
        const res = await fetch(`${API}/api/vswr?limit=200`);
        allData = await res.json();

        let cleared = allData.filter(r => r.clearance_status === 'Cleared').length;
        document.getElementById('cleared-vswr').textContent = cleared.toLocaleString();
